...
function TestComponent({
  ...
  return (
    <div>
      ...
      <button onClick={() => mgmt.openEditAttr({ id: 'a1', name: 'hostname', description: 'desc', type: 'string', ordinal: 0, required: true })}>Open Edit</button>
      <button onClick={() => mgmt.setEditAttrOpen(false)}>Close Edit</button>
      <button onClick={() => mgmt.setCopyAttrsOpen(true)}>Open Copy</button>
      <button onClick={() => mgmt.handleAddAttribute({ name: 'test', description: 'desc', type: 'string', enumId: '', required: false })}>Add Attr</button>
      ...
      <button onClick={() => mgmt.handleEditAttribute({ name: 'hostname2', description: 'updated', type: 'string', enumId: '', required: true })}>Edit Attr</button>
...
test('T-20.10: useAttributeManagement add modal toggles', async () => {
  render(<TestComponent entityTypeId="et-1" attributes={mockAttributes} enums={mockEnums} />)
