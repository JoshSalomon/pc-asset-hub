...
| TD-58 | 🔴 Enum values are not versioned — mutations are destructive | **RESOLVING via FF-14.** Enums are being replaced by versioned type definitions (US-5, US-44). Type definition versions are pinned in CVs, so mutations create new versions without affecting existing CVs. See `docs/superpowers/specs/2026-04-11-type-system-design.md`. | Will be marked ✅ resolved when FF-14 implementation is complete. |
